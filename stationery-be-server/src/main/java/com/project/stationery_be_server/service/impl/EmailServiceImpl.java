...
public class EmailServiceImpl implements EmailService {
    ...
    @Override
    public String sendSupportEmail(SupportEmailRequest request) {
        try {
            // Tạo email
            MimeMessage message = javaMailSender.createMimeMessage();
            MimeMessageHelper helper = new MimeMessageHelper(message, true, "UTF-8");
            ...
            helper.setFrom(sender);
            helper.setTo("nguyenquockhoa5549@gmail.com");  // Email của bộ phận hỗ trợ
            helper.setSubject(request.getSubject());

            // Nội dung email
            String htmlContent = "<div style='font-family: Arial, sans-serif; max-width: 600px; margin: auto; "
                    + "padding: 20px; border: 1px solid #ddd; border-radius: 10px;'>"
                    + "<h2 style='color: #333;'>Customer Support Request</h2>"
                    + "<p><strong>Full Name:</strong> " + request.getFullName() + "</p>"
                    + "<p><strong>Email Address:</strong> " + request.getEmailAddress() + "</p>"
                    + "<p><strong>Subject:</strong> " + request.getSubject() + "</p>"
                    + "<p><strong>Message:</strong></p>"
                    + "<p style='border-left: 4px solid #007BFF; padding-left: 10px;'>" + request.getMessage() + "</p>"
                    + "<hr style='border: 0; height: 1px; background: #ddd;'>"
